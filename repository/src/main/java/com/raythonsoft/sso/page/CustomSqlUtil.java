    ...
    public Object doIntercept(Invocation invocation) throws Throwable {
        ...
        if (!runtimeDialect.skip(ms, parameterObject, rowBounds)) {
            ...
            if (runtimeDialect.beforeCount(ms, parameterObject, rowBounds)) {
                ...
                if (countSql.contains(RepositoryConstant.SQL_SIGN)) {
                    Integer subIndex = countSql.indexOf(RepositoryConstant.SQL_SIGN) + RepositoryConstant.SQL_SIGN.length();// 截取位置 到AS limitable
            ...
            if (runtimeDialect.beforePage(ms, parameterObject, rowBounds)) {
                ...
                if (pageSql.contains(RepositoryConstant.SQL_SIGN)) {
                    String sqlInCount = pageSql.substring(0, pageSql.indexOf(RepositoryConstant.SQL_SIGN));
                    Integer count = -2;
