...
public class SessionOperationRepositoryImpl implements SessionOperationRepository {
    ...
    public void deleteClientSessionId(String sessionId) {
        // 局部会话checkCode
        String checkCode = codeRedisRepository.getCheckCodeByGenningSessionId(sessionIdGenerator.genClientSessionId(sessionId));
        // 删除局部会话
        redisTemplate.delete(sessionIdGenerator.genClientSessionId(sessionId));
        // 删除同一个code（server）注册的局部会话
        redisTemplate.opsForSet().remove(sessionIdGenerator.genClientSessionIdsCodeParamCode(checkCode), sessionId);
    }

    @Override
    public void deleteServerSessionId(String sessionId) {
        // 全局会话checkCode
        String checkCode = codeRedisRepository.getCheckCodeByGenningSessionId(sessionIdGenerator.genServerSessionId(sessionId));
        // 清除全局会话
        redisTemplate.delete(sessionIdGenerator.genServerSessionId(sessionId));
        // 清除code校验值
        codeRedisRepository.removeCheckCode(checkCode);
        ...
        // server下的client会话【们】
        String clientSessionIdsCode = sessionIdGenerator.genClientSessionIdsCodeParamCode(checkCode);
        ...
        codeRedisRepository.scardCacheCode(checkCode);// 删除Server后打印一下日志
