 ...
 */
@Controller
@RequestMapping("/sso/pre")
@Api(value = "单点登录管理(参考)")
@Log4j
public class SsoControllerPre {
    ...
    public String login(String backUrl) {
        ...
        // 全局会话checkCode
        String checkCode = codeRedisRepository.getCheckCodeByGenningSessionId(sessionIdGenerator.genCheckCode(serverSessionId));// 全局会话session
        ...
        if (StringUtils.isNotEmpty(checkCode)) {
            ...
            log.debug(String.format("认证中心账号通过，带code回跳：%s", backUrl));
    ...
    public Result login(@RequestParam String username, @RequestParam String password, @RequestParam(required = false, defaultValue = "false") boolean rememberMe, String backUrl) {
        ...
        Session session = subject.getSession();
        String serverSessionId = String.valueOf(session.getId());
        log.info("==> ServerSessionId: {}" + serverSessionId);

        String ossCode = codeRedisRepository.getCheckCodeByGenningSessionId(sessionIdGenerator.genCheckCode(serverSessionId));// 全局会话session
    ...
    public Result code(String checkCode) {
        String checkCodeFromRedis = codeRedisRepository.getCheckCodeByGenningSessionId(sessionIdGenerator.genCheckCode(checkCode));
